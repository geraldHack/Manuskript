...
/**
 * Lokale Speicherung des OpenRouter Management API Keys (nur für dieses Hilfsprogramm).
 */
public final class MonitorKeyStore {

    private static final String PREF_NODE = "/com/manuskript/openrouter/monitor";
    private static final String PREF_KEY = "management_api_key";
    private static final String ENV_KEY = "OPENROUTER_MANAGEMENT_API_KEY";
    private static final String FILE_NAME = "openrouter-monitor.properties";
    private static final String FILE_KEY = "management_api_key";

    private MonitorKeyStore() {
    }

    public static String loadManagementKey(Path configRoot) {
        ...
        if (isNonBlank(fromFile)) {
            return fromFile.trim();
        }
        return "";
    }

    public static void saveManagementKey(Path configRoot, String key) {
    ...
    private static void writeToFile(Path configRoot, String value) {
        ...
        try (Writer w = new OutputStreamWriter(new FileOutputStream(file), StandardCharsets.UTF_8)) {
            props.store(w, "OpenRouter Monitor – lokale Einstellungen");
        } catch (Exception ignored) {
        }
    }
