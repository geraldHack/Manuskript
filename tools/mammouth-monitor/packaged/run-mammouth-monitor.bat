@echo off
REM Startet den Mammouth-Monitor.

setlocal EnableExtensions EnableDelayedExpansion
set "SCRIPT_DIR=%~dp0"

set "JAR="
if exist "%SCRIPT_DIR%mammouth-monitor.jar" set "JAR=%SCRIPT_DIR%mammouth-monitor.jar"
if not defined JAR if exist "%SCRIPT_DIR%..\target\mammouth-monitor.jar" (
  for %%I in ("%SCRIPT_DIR%..\target\mammouth-monitor.jar") do set "JAR=%%~fI"
)
...
if not defined MODULE_DIR if exist "%CONFIG_DIR%\tools\mammouth-monitor\pom.xml" (
  set "MODULE_DIR=%CONFIG_DIR%\tools\mammouth-monitor"
)
if not defined MODULE_DIR (
  echo Mammouth-Monitor: weder gebuendelte JRE mit JavaFX noch Maven-Projekt gefunden.
  exit /b 1
)
